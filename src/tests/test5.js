...
export default function makeTests5() {
  ...
  describe('Task 5: Lucky Tickets (Counts lucky tickets from tickets range)', () => {
    ...
    describe('Tests with valid arguments', () => {
      ...
      it('Success: \'Complex Method\' won, simpleCount - 0, complexCount - 54 ({ min: 1, max: 1000, })', () => {
        chai.assert.deepEqual(task5({ min: 1, max: 1000, }),
          { winMethod: 'Complex Method', simpleCount: 0, complexCount: 54, });
      });

      it('Success: \'Complex Method\' won, simpleCount - 0, complexCount - 54 ({ min: 125, max: 2570, })', () => {
        chai.assert.deepEqual(task5({ min: 125, max: 2570, }),
          { winMethod: "Complex Method", simpleCount: 9, complexCount: 154, });
      });

      it('Success: \'Draw\', simpleCount - 55252, complexCount - 55252 ({ min: 0, max: 999999, })', () => {
        chai.assert.deepEqual(task5({ min: 0, max: 999999, }),
          { winMethod: 'Draw', simpleCount: 55252, complexCount: 55252, });
      });

      it('Success: \'Simple Method\' won, simpleCount - 31607, complexCount - 30213 ({ min: 123456, max: 654321, })', () => {
        chai.assert.deepEqual(task5({ min: 123456, max: 654321, }),
          { winMethod: 'Simple Method', simpleCount: 31607, complexCount: 30213, });
      });

      it('Success: \'Simple Method\' won, simpleCount - 1638, complexCount - 1497 ({  min: 37505, max: 68240, })', () => {
        chai.assert.deepEqual(task5({ min: 37505, max: 68240, }),
          { winMethod: "Simple Method", simpleCount: 1638, complexCount: 1497, });
      });
