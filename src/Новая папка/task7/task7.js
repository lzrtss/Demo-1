...
const isValid = function checkArgs(obj) {
  ...
  if (obj.min <= minValue || obj.min > maxValue
    ...
    || obj.min > obj.max || obj.length > maxLength
...
const getFiboRange = function getFiboInRangeOfMinMax(min, max) {
  const result = [];
  const fibArr = [0, 1];
  let currentFib = 0;
...
const getFiboLength = function getFiboOfRangeLength(length) {
  if (length < 2) {
    return [0];
  } else if (length == 2) {
    return fibArr;
...
// Main function
const showFiboSequence = function getFiboSequenceFromRange(obj) {
